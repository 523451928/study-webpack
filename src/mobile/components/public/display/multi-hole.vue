...
<div class="display-wrapper" :style="style.canvas">
  ...
  <div class="display-canvas">
    <div v-if="editstatus" class="display-back" v-for="b of style.back" :style="b.style" @touchstart.prevent.stop="evStart($event)" @touchmove.prevent.stop="evMove($event)"
         @touchend.prevent="evEnd($event)">
      <paint :size="b.size" :picture="b.picture"></paint>
    </div>
    <div v-else class="display-back" v-for="b of style.back" :style="b.style" >
      <paint :size="b.size" :picture="b.picture"></paint>
    </div>
...
export default {
  ...
  mounted() {
    if (_.isUndefined(this.hole)) this.changeHole(-1)
  },
  watch: {
    T: {
      immediate: true,
  ...
  methods: {
    changeHole(index) {
      ...
      this.$bus.$emit('change-hole', this.hole)
    }
  }
}
</script>
