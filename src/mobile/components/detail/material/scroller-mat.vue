...
<template>
<div class="size-scroller m-boms">
  <div class="mat-size" flex="dir:left cross:center">
    ...
    <div class="select-content swiper-container swiper-container-size1">
      <ul ref="size" class="scroll-list scroll-text-list clearfix swiper-wrapper">
        <li v-for="s of m2.sizes" :class="getSizeClass(s)" @click="selectSize(s, 'size')" class="size-item swiper-slide">
  ...
  <div class="mats-scroller mat-color">
    <overflow-box :bom-length="stack > 0 ? options.length : 0" v-ref:overflower flex="dir:left cross:center ">
      <div class="scroller-header clearfix">
        <span class="bom-type">卡纸:</span>
        <!-- <mat-stack :max="max" :stack="stack" :index="index"></mat-stack> -->
      </div>
      <!-- 形状 -->
      <div class="select-content swiper-container swiper-container-options">
        <ul v-if="stack > 0 && view === 'shape'" class="swiper-wrapper scroll-list scroll-icon-list clearfix" ref="shape">
          <li v-for="i of shapes" :class="getMixClass(i)" @click="selectMix(i)" :style="getMixStyle(i)" class="swiper-slide">
            <list-item :id="i.id" :name="i.name" :icon="i.icon"></list-item>
          </li>
        </ul>
        <!-- 卡纸颜色 -->
        <ul v-if="stack > 0 && view === 'color'" class="swiper-wrapper scroll-list scroll-icon-list clearfix" ref="color">
          <li v-for="i of options" :class="getOptionClass(i)" @click="changeOption('set', i)" class="color-item swiper-slide">
...
export default {
  ...
  computed: {
    ...
    size() {
      ...
      switch (x.mat.Type) {
        case mt.Size:
          ...
          break
        case mt.Mat:
          // 如果角度是90或者180度 则给尺寸做一个旋转,如果没有旋转,则返回一个尺寸
    ...
    m2() {
      ...
      // let _sizes2 = _sizes.filter(v => {
      //   for (let j of _m) {
      //   let s = j.Properties.Size
      //     if (v[0] === s[0] && v[1] === s[1]) {
      //       return v
      //     }
      //   }
  ...
  methods: {
    ...
    initSwiper() {
      this.$nextTick(() => {
        let swiper = new Swiper('.swiper-container-size1', {
          autoplay: false,
          slidesPerView: 'auto',
          slidesPerGroup: 4,
          spaceBetween: 20,
        ...
        let swiper2 = new Swiper('.swiper-container-options', {
          autoplay: false,
          slidesPerView: 'auto',
          slidesPerGroup: 4,
          setWrapperSize: true
