...
import { DecorativeCategories as dc, MaterialTypes as mt,
  DisplayTypes as dt, Angles, StackLines, Fulls, isDecorate,
  tabs, product, SuggestSize as ss
} from 'scripts/utils/custom/consts.js'
import { getMaterials } from './material.js'
import { zeroEntry, freshEntry, freshPhotoWall, freshMatOptions,
  freshScrollCanvas,
} from 'components/custom/core/entry.js'
import { zeroArray, getByProperty, setByProperty, indexOfProperty, cleanBlank,
  equalSize, swapSize, parseImageSize, reversePicturesData, resolveEntry } from 'scripts/utils/custom/utils.js'
import { getVirtualSize, fitSize, newPicture, serializePictures } from '../picture/picture'
      ...
      selectMaterial(material, triggerBom, callback) {
        ...
        if (t === 'image') {
          ...
          if (material) {
            ...
            if (callback) {
              this.selectImage(image, callback)
            } else {
