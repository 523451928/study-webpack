...
<div class="display-wrapper" :style="style.canvas">
  <div class="display-canvas">
    <div v-for="p of style.photos" :style="p.style" @click="changePhoto(p.index)">
      <component :keep-alive="false" :is="p.displayer" :entry="p.entry"
...
import { DecorativeCategories as dc, DisplayTypes as dt,
  StackLines as sl } from './consts'
import { mixin, getDisplayer } from './display-util'
...
export default {
  ...
  methods: {
    changePhoto(index) {
      this.$EE.emit('change-indicator', index + 1)
    }
  }
}
</script>
