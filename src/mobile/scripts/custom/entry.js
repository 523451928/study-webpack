...
import { DecorativeCategories as dc, MaterialTypes as mt,
  isDecorate, getTypes, Angles, StackLines as sl } from './consts'
import { zeroArray, indexOfProperty, getByProperty, setByProperty,
  cleanBlank, equalOption, equalSize, similarSize, copySize } from './util'
import { allMaterials } from './material'
import { ngParamSerializer } from 'scripts/_serializer'
import { openDialog } from 'components/prompt'

const DefaultSize = [400, 400]
...
function freshDecorative(materials, entry, fix) {
  ...
  switch (ci) {
    ...
    case dc.Poster:
      ...
      b.push({Type: 'texture', Value: o})
      b.push({Type: 'blank', Value: cleanBlank(b[a.indexOf(mt.Size)].Size, d.Blank)})
  ...
  return new Promise((resolve, reject) => {
    ...
    allMaterials(photos.map(v => v.CategoryId)).then(data => {
      ...
      for (i = 0; i < photos.length; i++) {
        ...
        data[i] = {categoryId: p.CategoryId, materials: a, selects: freshDecorative(a, e, s)}
      }
  ...
  for (let i of selects) {
    switch (i.Type) {
      case 'mat':
        ...
        break
      case 'line':
        if (i.inner.Id) a.push(i.inner.Id)
        if (i.outer.Id) a.push(i.outer.Id)
        break
      case 'texture':
        a.push(i.Value.Id)
        break
