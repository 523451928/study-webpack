...
function load(ids, costStatus) {
  return request.getEntry('entry', ids, costStatus).then(res => {
    const data = res.take() || []
...
function loads(ids, costStatus, seq = true) {
  let a = []
  let f, p
  if (seq) {
    p = Promise.resolve(seq)
    f = v => {
      p = p.then(() => load(v, costStatus))
      a.push(p)
    }
  } else {
    f = v => a.push(load(v, costStatus))
  }
...
export function getEntries(ids, costStatus) {
