  ...
  <div class="main-cont tab-content col-md-10">
    <div class="form-wrap">
      <div class="panel panel-default">
        <div class="panel-heading">绑定账号</div>
        <div class="panel-body social">
          <ul class="list-unstyled social-ac-list">
            ...
            <li v-else>
              ...
              <span class="item pull-right"><a :href="`${baseUrl}/social/bind/qq?Link=${params.Link}`" class="ope" target="_blank">去绑定</a></span>
            </li>
            ...
            <li v-else>
              ...
              <span class="item pull-right"><a :href="`${baseUrl}/social/bind/weibo?Link=${params.Link}`" class="ope" target="_blank">去绑定</a></span>
            </li>

            <li v-if="weixin && weixin['Id']">
              <span class="item stat"><i class="wa wa-wechat"></i> 已绑定微信账号：{{weixin['Nickname']}}</span>
              <span class="item pull-right"><a href="javascript:void(0);" class="ope" @click="unbind(weixin['Id'])">解除绑定</a></span>
            </li>
            <li v-else>
              <span class="item icon"><i class="wa wa-wechat"></i> 微信账号未绑定</span>
              <span class="item pull-right"><a :href="`${baseUrl}/social/bind/weixin?Link=${params.Link}`" class="ope" target="_blank">去绑定</a></span>
            </li>
          </ul>
          ...
          <!--<label class="checkbox-inline text-left" id="weiboCheck" for="weibo">-->
            ...
            <!--</template>-->
            <!--<template v-else>-->
              <!--<i class="wa wa-weibo"></i><span class="text-lowercase">请先绑定微博账号</span>-->
            <!--</template>-->
          <!--</label>-->
          ...
          <!--<template v-else>-->
            <!--<label class="checkbox-inline text-left" for="weixin">-->
              <!--<i class="wa wa-wechat"></i> <span class="text-lowercase">请先绑定微信账号</span>-->
            <!--</label>-->
          <!--</template>-->
        </div>
      </div>
      ...
      upload($event) {
        ...
        _.each(b, function(v, i) {
          ...
          request.submit('upload/wxhp', fd, {
            ...
            self.weixinFile = file
            self.weixinpic = `${Interf.baseUrl}/thumbnail/wxhp/${self.$user.UserId}/480/${file}`
            layer.toast('上传成功')
          }, res => {
            res.hide()
