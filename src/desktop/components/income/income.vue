        ...
        <div class="panel-body min">
          <div class="panel-hd mb20 mt15">
            已核实收入<strong class="color-d1 u-balance">{{incomeAll.FinishIncome/100 || 0}}</strong>元，<span v-if="able" class="withdraw-now">可提现收入<strong class="color-d1 u-wait">{{incomeAll.Available/100 || 0}}</strong>元。
                                    ...
                                    <a class="btn btn-default btn-lg income-btn" href="/withdraw-list.html">提现列表</a>
                                </span>
          ...
          <p class="tips">由于提现涉及现金操作，为了保证用户数据和财产的安全性，请先在 <a href="./setting.html#/creatShop">开店认证</a> 中进行实名认证和手机认证。</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      pull() {
        request({url: 'income/profit/balance'}).then(res => {
