...
<div class="display-wrapper" :style="style.canvas">
  ...
  <div class="display-canvas">
    <div class="display-back" :class="editstatus ? 'ed' : ''" :style="style.back">
    ...
    <div v-if="editstatus" class="display-fore ed" @mouseover="evBeforeStart()"  @mousedown.prevent="evStart($event)" @mousemove.prevent.stop="evMove($event)"
         @mouseleave.prevent="evEnd($event)" @mouseup.prevent="evEnd($event)" :style="style.fore"></div>
    <div v-else class="display-fore" :style="style.fore">
    </div>
...
export default {
  ...
  watch: {
    T: {
      immediate: true,
      handler(t) {
        t.d && this.loadImages([t.d.Fore, t.d.Back])
      }
    }
  },
  computed: {
    T() {
      return {
        id: this.entry.Id,
        l: this.el,
        d: this.displays && this.displays[this.realDi]
      }
    },
    style() {
      let u = this.addUnit
