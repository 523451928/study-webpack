      ...
      <div class="container">
        <div id="photo-default-bar" v-show="list">
          <div class="pull-left back-to-album">
            <a class="primary-link back-to-album-link" href="/user/album.html#/album">&lt;返回作品集列表</a>
          </div>
    ...
    <div class="cont-wrap photo-wrap mb65" id="photo-list-box">
      ...
      <div v-show="isFull && list" id="photo-gallery2" class="photo-gallery flex-images container">
        <div class="pto-list dn item" :data-w="calculate(item['File'],'w')" :data-h="calculate(item['File'],'h')" @click="preview(index)" v-for="(item,index) in obj">
          <!--<img v-lazyload="`${baseUrl}/thumbnail/image/${item['Id']}/480/${item['File']}`">-->
          <img :src="`${baseUrl}/thumbnail/image/${item['Id']}/480/${item['File']}`">
          <div class="photo-hover-box">
            ...
            <div class="ww-photo-hover-detail">
              <div class="ww-photo-hover-data">
                ...
                <div class="ww-photo-hover-time" v-if="item['CreateTime']" v-text="item['CreateTime'].substring(0,10)"></div>
              </div>
      ...
      <div id="photo-icon" v-show="!list" class="photo-gallery container">
        <ul class="list-unstyled cont-list batch-list clearfix">
          <li :class="x(i)?'active':''" @click="iconToggle(i)" v-for="(i,index) in obj">
            ...
            <a class="batch-img-list" :style="{backgroundImage: `url(${baseUrl}/thumbnail/image/${i['Id']}/240/${i['File']})`}">&nbsp;</a>
          </li>
        </ul>
      </div>
      <!-- 缩略图结束-->
                      ...
                      <li v-for="(item,index) in obj">
                        <div class="pto-list">
                          <div class="real-img-box">
                            <img draggable="false" :alt="item['Title']?item['Title']:item['Name']?item['Name']:'默认相册'" :style="{transform:'rotate('+(index==serial?1:0)*(serialDegree*90)+'deg)'}" :src="`${baseUrl}/thumbnail/image/${item['Id']}/480/${item['File']}`"/>
                  ...
                  <div class="flex-viewport">
                    <ul class="slides carousel-list" :style="{width: '400%',transform: 'translate3d('+(serial*65>correct?-serial*65+correct:0) +'px, 0px, 0px)'}">
                      <li v-for="(item,index) in obj" @click="toNum(index)">
                        <div class="pto-list">
                          <img draggable="false" :src="`${baseUrl}/thumbnail/image/${item['Id']}/120/${item['File']}`" :alt="item['Title']?item['Title']:item['Name']?item['Name']:'默认相册'" />
                        </div>
                      </li>
                    </ul>
                  </div>
                ...
                <ul class="ww-photo-edit-list">
                  <li>
                    ...
                    <select class="form-control photo-album ww-photo-edit-area" v-model="serialAlbum">
                      <option :value="item['Id']" :selected="index == 0 ? 'selected' : ''" v-for="(item, index) in objListAll">{{item['Name']?item['Name']:'默认相册'}}</option>
                    </select>
                  </li>
                  <li>
                    <label>图片标题</label>
                    <div class="input-group ww-photo-group">
                      <input type="text" v-model="serialTitle" class="form-control photo-title ww-photo-edit-area" placeholder="请填写图片标题" aria-describedby="basic-addon1"/>
                    </div>
                  </li>
                  <li>
                    <label>标签</label>
              ...
              <ul class="ww-add-album-eidt">
                <li class="clearfix">
                  ...
                  <select class="form-control photo-album ww-photo-edit-area" v-model="ToAlbumId">
                    <option :value="item['Id']" :selected="item['Name'] === '默认相册' ? 'selected' : ''" v-for="item in objListAll">{{item['Name']?item['Name']:'默认相册'}}</option>
                  </select>
                </li>
              </ul>
      ...
      pullList() {
        ...
        request({url: 'image/album/list'}).then(res => {
          ...
          if (data) {
            ...
            // console.log('self.objListAll:', self.objListAll)
...
//            }
      ...
      initSerial(index) {
        // console.log('imgindex:', this.obj[index])
        let title = this.obj[index]['Name']
        this.serialTitle = this.obj[index]['Title'] || title.slice(0, title.lastIndexOf('.'))
