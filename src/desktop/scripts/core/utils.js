...
// 物料去重
export function wipeRepetition(items) {
  if (!Array.isArray(items)) return false
  let r = []
  for(let k of items) {
    let id = k.Id
    if (r.every(v => v.Id !== id)) {
      r.push(k)
      break
    }
  }
...
/**
 * 根据品类id,对物料做一次复制操作, 将尺寸方向不同的物料做一次过滤操作
 * @param {Array} materlist 物料数组
 */
export let copyMaterials = (materlist) => {
  let r = []
  materlist.forEach((item ,index) => {
    let copy
    let s
    switch(item.Type) {
      case mt.Mat:
        r.push(item)
        s = item.Properties.Size
        if (s[0] !== s[1]) {
          copy = JSON.parse(JSON.stringify(item))
          copy.Properties.Size = swapSize(s)
          r.push(copy)
        }
        break
      case mt.Size:
...
/**
 * 获得所有物料后,对所有品类的物料数据进行复制操作
 * @param {Array} allMaterial 所有物料所组成的数组
 */

export let traversalMaterials = (allMaterial) => {
  let r = []
  allMaterial.forEach(item => {
    r.push(copyMaterials(item))
  })
  return r
}